...
entity rtc_master is
  ...
  port (
    ...
    rtc_read_valid_o : out std_logic;
...
architecture synthesis of rtc_master is
  ...
  -- External replacement RTC supplied for affected R3/R3A boards:
  -- RTC device   : DS3231
  -- I2C bus      : Grove connector
  -- I2C address  : 0x68 (write 0xD0, read 0xD1)
  -- I2C register : 0x00
  -- I2C byte_cnt : 0x07
  -- Register order: seconds, minutes, hours, weekday, date, month, year.
  constant C_ACTION_LIST_READ_R3_GROVE : action_list_t := (
    0 => (WAIT_CMD,     X"F1", X"0001"),
    1 => (WRITE_CMD,    X"00", X"0000"),
    2 => (WRITE_CMD,    X"F0", X"01D0"),
    3 => (WAIT_CMD,     X"F1", X"0000"),
    4 => (WAIT_CMD,     X"F1", X"0001"),
    5 => (WRITE_CMD,    X"F0", X"07D1"),
    6 => (WAIT_CMD,     X"F1", X"0000"),
    7 => (WAIT_CMD,     X"F1", X"0001"),
    8 => (SHIFT_IN_CMD, X"00", X"0004"),
    9 => (VERIFY_CMD,   X"00", X"0000")
   );

  constant C_ACTION_LIST_WRITE_R3_GROVE : action_list_t := (
    0 => (WAIT_CMD,      X"F1", X"0001"),
    1 => (SHIFT_OUT_CMD, X"00", X"0004"),
    2 => (WRITE_CMD,     X"F0", X"08D0"),
    3 => (WAIT_CMD,      X"F1", X"0000"),
    4 => (WAIT_CMD,      X"F1", X"0001"),
  ...
  begin
    ...
    elsif board = "MEGA65_R3_GROVE" then
      return C_ACTION_LIST_READ_R3_GROVE;
    else
  ...
  begin
    if board = "MEGA65_R3" then
      ...
      return C_ACTION_LIST_WRITE_R3_GROVE;
    else
  ...
  begin
    if board = "MEGA65_R3" then
      ...
      -- DS3231 order is SEC, MIN, HOUR, DOW, DATE, MONTH, YEAR.
      -- Convert it to the common RTC layout used by the Nascom core:
      -- DOW, YEAR, MONTH, DATE, HOUR, MIN, SEC, hundredths.
      return arg(31 downto 24) & arg(55 downto 32) &
        arg(23 downto 0) & X"00";
    else
  ...
  begin
    if board = "MEGA65_R3" then
      ...
      -- Common layout -> DS3231 register order, prefixed with register 0.
      return X"00" & arg(55 downto 32) & arg(63 downto 56) &
        arg(31 downto 8) & X"00";
    else
      -- Valid for R4, R5, and R6
  ...
  signal transaction_failed : std_logic := '0';
  signal rtc_read_valid : std_logic := '0';
...
begin
  ...
  rtc_read_valid_o <= rtc_read_valid;
      ...
      case state is
            ...
            case action.cmd is
              ...
              when WAIT_CMD =>
                ...
                if cpu_m_ce_o = '1' and cpu_m_wait_i = '0' then
                  -- Status bit 2 is the sticky I2C NACK indication. Reading
                  -- the status register clears it, so remember it for the
                  -- complete RTC transaction before continuing.
                  if action.addr = X"F1" and cpu_m_rd_data_i(2) = '1' then
                    transaction_failed <= '1';
                  end if;
                  if cpu_m_rd_data_i = action.data then
                    next_action <= '1';
                  end if;
                end if;
        ...
        when VERIFY_ST =>
          ...
          if transaction_failed = '1' then
            -- A missing device must complete as an invalid read instead of
            -- retrying forever. This is required for automatic bus fallback.
            rtc_read_valid <= '0';
            state <= IDLE_ST;
          elsif rtc(47 downto 32) = X"0000" then
